...
class GestureClassifier(QThread):
    ...
    def __init__(self, args):
        ...
        self.instructions = [
            "Press 'Space bar' 25 times for left gesture",
            "Press 'Space bar' 25 times for right gesture",
            "Press 'Space bar' 25 times for up gesture",
            "Press 'Space bar' 25 times for down gesture",
            "Press 'Space bar' 25 times for open gripper gesture",
            "Press 'Space bar' 25 times for close gripper gesture"
        ]
    ...
    def write_current_user(self):
        ...
        log_file_path = os.path.join(script_dir, 'currentUserLog.txt')  # join the script path with the log file path

        print(f"This is the MAILLLLL: !!!!!!!!!!{self.user_email}")
    ...
    def check_if_model_exists(self):
        ...
        model_directory = './utils/keypoint_classifier/tflite_models'  # Replace with your path
    ...
    def clear_csv(self):
        ...
        csv_path = os.path.join(script_dir, './utils/keypoint_classifier/keypoint.csv')  # join the script path with the relative csv file path
    ...
    def play_video(self, file_path):
        ...
        # Desired output frame width
        frame_width = 400
        # Define frame limit for 5 seconds (assuming video is 30 FPS)
        frame_limit = 6 * 30  # Adjust the multiplier (5 here) for the number of seconds
        frame_count = 0
        ...
        while video.isOpened() and frame_count < frame_limit:
            ...
            if ret:
                # Calculate the ratio of the old height to the new height, clone it, and resize it
                ratio = frame_width / float(frame.shape[1])
                dim = (frame_width, int(frame.shape[0] * ratio))
    ...
    def run_ros_commands(self, user_email):
        if self.modelHasBeenGenerated:
            # Run gesture_recognition.launch with email argument
            subprocess.Popen(["roslaunch", "robot_control", "gesture_recognition.launch", f"email:={user_email}"])
        else:
            print("Model has not been generated yet.")
    ...
    def run(self):
        ...
        # Read labels ###########################################################
        script_dir = os.path.dirname(os.path.realpath(__file__))  # get the path of the script
        label_file_path = os.path.join(script_dir, 'utils/keypoint_classifier/keypoint_classifier_label.csv')  # join the script path with the relative label file path
        ...
        while self.ThreadActive: # Check if the thread is supposed to be running
            ...
            if self.modelCreation and not self.modelHasBeenGenerated: #This happens once, while model is generating
                ...
                # Reset the frame counter when the model is generated
                frame_counter = 0

            # Increment the frame counter if the model has been generated
            if self.modelHasBeenGenerated:
                frame_counter += 1

            # Run the ROS commands after 30 seconds (assuming 30 frames per second)
            if self.modelHasBeenGenerated and frame_counter == (12.6) * 30:
                self.run_ros_commands(self.user_email)
                self.ThreadActive=False
                self.cap.release()
                cv.destroyAllWindows()
            ...
            if key == 107:  # k
                ...
                if self.current_instruction < len(self.instructions):
                    ...
                    instruction_video = os.path.join(script_dir, "Command Videos", "Left.MOV")
                    self.play_video(instruction_video)
            ...
            if results.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                    ...
                    if key == 32 and self.mode == 1:  # space
                        ...
                        if self.tab_presses[self.current_instruction] >= 25:
                            self.current_instruction += 1
                            self.countUpdate.emit(self.current_instruction)  # emit the signal
                            if self.current_instruction < len(self.instructions):  # If there are more instructions
                                QMessageBox.information(None, "Instructions", self.instructions[self.current_instruction])
                                
                                # Showing current video
                                video_name = self.command_videos[self.current_instruction] + ".MOV"
                                script_dir = os.path.dirname(os.path.abspath(__file__))  # get the directory of the script
                                # Join the directory with the video file path
                                instruction_video = os.path.join(script_dir, "Command Videos", video_name)
                                self.play_video(instruction_video)
...
#roslaunch robot_control gesture_recognition.launch email:=user@example.com
...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.emailLabel = QLabel("Choose Username")
