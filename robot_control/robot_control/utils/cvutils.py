...
class CvUtils():
  ...
  # Add an attribute to store the number of frames since the last recognition

  def __init__(self, email=None, video_src=VIDEO_SRC):
    ...
    # Update recognition cooldown value
  ...
  def process_stream(self):
    ...
    # # Calculate FPS (you need to implement this according to your system)
   ...
   # Hands detections
    ...
    if len(self.keypoint_predictions) >= 30 and len(set(self.keypoint_predictions[-30:])) == 1:
        print("MORE THAN 30 ---------------------> ")
        predicted_action = self.keypoint_predictions[-1]
        # If the list is empty, or the last action is different, or enough frames have passed since the last recognition, append the new action
        if len(self.sentence) == 0 or self.frames_since_last_recognition >= self.recognition_cooldown:
          print("COOLD DOWN, APPEND TO THE SENTENCE LIST")
          print("Append gesture to the list: ", predicted_action)
          self.sentence.append(predicted_action)
          self.keypoint_predictions = []
          # Reset the counter
          self.frames_since_last_recognition = 0
        else:
          # If the gesture is not recognized, increase the counter
          self.frames_since_last_recognition += 1
    else:
      # If no gesture is detected, increase the counter
      self.frames_since_last_recognition += 1
  ...
  def load_keypoint_classifier(self):
    ...
    self.keypoint_classifier = KeyPointClassifier(
        email=self.email,
        model_path=os.path.join(BASE_DIR, 'keypoint_classifier', 'tflite_models','keypoint_classifier.tflite'),
    )
