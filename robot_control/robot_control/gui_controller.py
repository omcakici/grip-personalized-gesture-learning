...
from robot_control.msg import BotState
from robot_control.msg import GuiCmd
...
# from robot_control.sim_controller import AutoName
...
class GuiController:
    def __init__(self):
        ...
        self.command_publisher = rospy.Publisher("gui", GuiCmd, queue_size=10)
        self.demo_publisher = rospy.Publisher("demo", String, queue_size=10)
        self.bot_state_subscription = rospy.Subscriber(
            "bot_state", BotState, self.bot_state_cb
        )
        self.bot_state = BotState()

    def bot_state_cb(self, msg):
        rospy.loginfo(f"Incoming bot state {msg}")
        self.bot_state = msg
...
class Gui(App):
    ...
    def __init__(self):
        ...
        self.mode_label = Label(
            text=self.get_mode_label_txt(),
            pos=(80, self.HEIGHT - 40),
            size_hint=(0.3, 0.1),
            font_size=12,
        )

        self.subject_name_label = Label(
            text=self.subject_name,
            pos=(200, self.HEIGHT - 40),
            size_hint=(0.3, 0.1),
            font_size=12,
        )

        self.test_shape_label = Label(
            text=self.test_shape,
            pos=(340, self.HEIGHT - 40),
            size_hint=(0.15, 0.1),
            font_size=12,
        )

        self.depth_label = Label(
            text=self.get_depth_txt(),
            pos=(340, self.HEIGHT - 60),
            size_hint=(0.15, 0.1),
            font_size=12,
        )

        self.max_angle_label = Label(
            text=self.get_max_angle_txt(),
            pos=(340, self.HEIGHT - 80),
            size_hint=(0.15, 0.1),
            font_size=12,
        )

        self.command_label = Label(
            text="SIM Commands", pos=(25, self.HEIGHT - 90), size_hint=(0.30, 0.1)
        )

        self.cmd_dropdown_btn = Button(
            text="Cmd",
            pos=(40, self.HEIGHT - 120),
            size_hint=(0.20, 0.1),
            background_color=(0.1, 0, 1, 1),
        )
        self.cmd_dropdown_btn.bind(on_release=self.cmd_dropdown.open)
        self.cmd_dropdown.bind(
            on_select=lambda instance, x: setattr(self.cmd_dropdown_btn, "text", x)
        )

        self.shapes_dropdown_btn = Button(
            text="Shape", pos=(10, self.HEIGHT - 160), size_hint=(0.15, 0.1)
        )
        self.shapes_dropdown_btn.bind(on_release=self.shapes_dropdown.open)
        self.shapes_dropdown.bind(
            on_select=lambda instance, x: setattr(self.shapes_dropdown_btn, "text", x)
        )

        self.mods_dropdown_btn = Button(
            text="Mod", pos=(90, self.HEIGHT - 160), size_hint=(0.15, 0.1)
        )
        self.mods_dropdown_btn.bind(on_release=self.mods_dropdown.open)
        self.mods_dropdown.bind(
            on_select=lambda instance, x: setattr(self.mods_dropdown_btn, "text", x)
        )

        self.objs_dropdown_btn = Button(
            text="Object", pos=(10, self.HEIGHT - 200), size_hint=(0.15, 0.1)
        )
        self.objs_dropdown_btn.bind(on_release=self.objs_dropdown.open)
        self.objs_dropdown.bind(
            on_select=lambda instance, x: setattr(self.objs_dropdown_btn, "text", x)
        )

        self.args_input = TextInput(
            text="arg0 arg1 arg2",
            multiline=False,
            pos=(10, self.HEIGHT - 240),
            size_hint=(0.35, 0.1),
        )
        ...
        self.video_label = Label(
            text="Demo GUI", pos=(25, self.HEIGHT - 320), size_hint=(0.3, 0.1)
        )

        self.vids_dropdown_btn = Button(
            text="Video", pos=(10, self.HEIGHT - 350), size_hint=(0.15, 0.1)
        )
        self.vids_dropdown_btn.bind(on_release=self.vids_dropdown.open)
        self.vids_dropdown.bind(
            on_select=lambda instance, x: setattr(self.vids_dropdown_btn, "text", x)
        )

        self.update_vid_button = Button(
            text="Update vid",
        ...
        self.timer_label = Label(
            text=self.get_timer_elapsed_time_str(),
            pos=(370, self.HEIGHT - 120),
            size_hint=(0.15, 0.1),
        )

        self.reset_timer_button = Button(
            text="Reset timer", pos=(270, self.HEIGHT - 160), size_hint=(0.25, 0.1)
        )
        self.reset_timer_button.bind(on_press=self.reset_timer_pressed)

        self.extra_note_text_input = TextInput(
            text="Extra notes",
            multiline=False,
            pos=(270, self.HEIGHT - 200),
            size_hint=(0.25, 0.1),
        )

        self.save_to_file_button = Button(
            text="Save test results",
            pos=(250, self.HEIGHT - 240),
            size_hint=(0.35, 0.1),
            background_color=(0.7, 0.5, 0.1, 1),
        )
        self.save_to_file_button.bind(on_press=self.save_button_pressed)

        self.rmse_label = Label(
            text="RMSE", pos=(290, self.HEIGHT - 330), size_hint=(0.15, 0.1)
        )
        self.time_taken_label = Label(
            text="Time taken", pos=(290, self.HEIGHT - 300), size_hint=(0.15, 0.1)
        )
    ...
    def save_time_to_file(self):
        ...
        note = (
            self.extra_note_text_input.text
    ...
    def check_bot_state(self, dt):
        ...
        if bot_state:
            ...
            if bot_state.mode_changed and bot_state.mode_name != self.mode:
    ...
    def send_cmd_pressed(self, instance):
        ...
        self.gui_controller.publish_command(cmd, shape, mods, obj, args)
